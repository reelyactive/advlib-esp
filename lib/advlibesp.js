/**
 * Copyright reelyActive 2022-2024
 * We believe in an open Internet of Things
 */


const header = require('./header');
const radioERP1 = require('./radioerp1');
const utils = require('./utils');


const MIN_PACKET_LENGTH_BYTES = 8;
const MIN_ERP1_PAYLOAD_LENGTH_BYTES = 6;
const SYNC_BYTE = 0x55;
const CRC_TABLE = [
	  
];


/**
 * Process a raw EnOcean Serial Protocol packet into semantically meaningful
 * information.
 * @param {Object} data The raw ESP packet as a hexadecimal-string or Buffer.
 * @param {Array} libraries Optional ordered list of processing libraries.
 * @param {Object} options Optional processing options.
 * @return {Object} The processed packet as JSON.
 */
function process(data, libraries, options) {
  options = options || {};

  let buf = utils.convertToBuffer(data);
  let processedPacket = {};
  let isERP1PayloadOnly = (options.isERP1PayloadOnly === true);

  if((buf === null) ||
     (isERP1PayloadOnly && (buf.length < MIN_ERP1_PAYLOAD_LENGTH_BYTES)) ||
     (buf.length < MIN_PACKET_LENGTH_BYTES)) {
    return null;
  }

  if(isERP1PayloadOnly) {
    processedPacket = radioERP1.process(data, libraries, options);

    if(!processedPacket.hasOwnProperty('uri')) {
      processedPacket.uri =
                      'https://sniffypedia.org/Organization/EnOcean_Alliance/';
    }

    return processedPacket;
  }

  // Abort if any of the SYNC byte, CRC8H or CRC8D are invalid
  if((buf.readUInt8(0) !== SYNC_BYTE) || !isValidCRC(buf)) {
    return null;
  }

  let headerProperties = header.process(buf.subarray(1, 5));
  let dataBuf = buf.subarray(6, 6 + headerProperties.dataLength);
  let includeProtocolOverhead = (options.ignoreProtocolOverhead !== true);

  if(includeProtocolOverhead) {
    Object.assign(processedPacket, headerProperties);
  }

  // Process the packet based on its type
  switch(headerProperties.type) {
    case 'RADIO_ERP1':
      let telegramProperties = radioERP1.process(dataBuf, libraries, options);
      Object.assign(processedPacket, telegramProperties);
      break;
  }

  if(!processedPacket.hasOwnProperty('uri')) {
    processedPacket.uri =
                      'https://sniffypedia.org/Organization/EnOcean_Alliance/';
  }

  return processedPacket;
}


/**
 * Check that the CRC8H and CRC8D are both valid.
 * @param {Buffer} packet The raw ESP packet.
 * @return {boolean} Whether both are valid or not.
 */
function isValidCRC(packet) {
  let crc8h = packet.readUInt8(5);
  let crc8d = packet.readUInt8(packet.length - 1);
  let headerBytes = packet.subarray(1, 5);
  let dataBytes = packet.subarray(6, packet.length - 1);

  return (crc8h === calculateCRC(headerBytes)) &&
         (crc8d === calculateCRC(dataBytes));
}


/**
 * Calculate the expected CRC based on the given bytes.
 * @param {Buffer} bytes The packet bytes on which to calculate the CRC.
 * @return {Number} The expected CRC.
 */
function calculateCRC(bytes) {
	let crc = 0;

	for(let cByte = 0; cByte < bytes.length; cByte++) {
		crc = CRC_TABLE[(crc ^ bytes.readUInt8(cByte))];
	}

	return crc;
}


module.exports.process = process;
module.exports.header = header;
